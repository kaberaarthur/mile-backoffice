...
function Main() {
  ...
  const [driverProfile, setDriverProfile] = useState<DocumentData[]>([]);
    ...
    try {
      ...
      if (querySnapshot.empty) {
        ...
        querySnapshot.forEach((doc) => {
          const driverData = doc.data() as DocumentData;
          driverData.id = doc.id; // Include doc.id in the data object
          results.push(driverData);

          const data = doc.data() as DocumentData | undefined; // Explicitly cast to DocumentData or undefined
          if (driverData) {
            setDriverProfile([driverData]); // Update the state with the array containing the single data element
          } else {
  ...
  return (
    <>
      <div className="flex items-center mt-8">
        <h2 className="mr-auto text-lg font-medium intro-y">Search Driver</h2>
      </div>
      {/* BEGIN: Wizard Layout */}
      <div className="py-10 mt-2 intro-y box sm:py-20">
        <div className="px-5 mt-2">
          <div className="text-4xl font-medium text-center">Search Driver</div>
      ...
      <div>
        {/* Conditionally render elements based on whether DriverProfile is empty */}
        {driverProfile.length === 0 ? (
          ...
          <div>
            {/* Render other elements based on the properties in the driverProfile object */}
                    ...
                    <Preview>
                      <div className="overflow-x-auto">
                        <Table striped>
                          ...
                          <Table.Tbody>
                            <Table.Tr>
                              <Table.Td>{driverProfile[0].id}</Table.Td>
                              <Table.Td>{driverProfile[0].name}</Table.Td>
                              <Table.Td>{driverProfile[0].email}</Table.Td>
                              <Table.Td>{driverProfile[0].phone}</Table.Td>
                              <Table.Td>
                                {driverProfile[0].referralCode || "None"}
                              </Table.Td>
                              <Table.Td>
                                {driverProfile[0].activeUser
                                  ? "Active"
                                  : "Inactive"}
                              </Table.Td>
                              <Table.Td>
                                <Link
                                  to={`/view-driver/${driverProfile[0].id}`}
                                >
                                  <Button
                                    variant="primary"
                                    className="w-24 ml-2"
